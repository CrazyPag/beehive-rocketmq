package com.beehive.rocketmq.customer.repository;
...
public class ESRepository {
     ...
     * @return
     */
    public String addJSONDoc(String index, String type, JSONObject json) {
        client.prepareIndex().setIndex(index).setType(type).setSource();
        IndexResponse indexResponse = client.prepareIndex().setIndex(index).setType(type)
                .setSource(json, XContentType.JSON).get();
        log.debug("添加document，index:" + index + ",type:" + type + ",目标类obj:" + json);
        return indexResponse.getId();
    }
